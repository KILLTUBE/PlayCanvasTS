...
/// <reference path="../shape/bounding-box.ts" />


namespace pc {

    // Global shadowmap resources
    export var scaleShift = new pc.Mat4().mul2(
        new pc.Mat4().setTranslate(0.5, 0.5, 0.5),
        new pc.Mat4().setScale(0.5, 0.5, 0.5)
    );

    export var opChanId = { r: 1, g: 2, b: 3, a: 4 };

    export var pointLightRotations = [
    ...
    export var numShadowModes = 5;
    export var shadowMapCache = [{}, {}, {}, {}, {}]; // must be a size of numShadowModes

    export var directionalShadowEpsilon = 0.01;
    export var pixelOffset = new Float32Array(2);
    export var blurScissorRect = { x: 1, y: 1, z: 0, w: 0 };

    export var shadowCamView = new pc.Mat4();
    export var shadowCamViewProj = new pc.Mat4();
    export var c2sc = new pc.Mat4();

    export var viewInvMat = new pc.Mat4();
    ...
    export var viewProjMatR = new pc.Mat4();
    ...
    export function _getFrustumPoints(camera, farClip, points) {
    ...
    export var _sceneAABB_LS = [
        new pc.Vec3(), new pc.Vec3(), new pc.Vec3(), new pc.Vec3(),
        new pc.Vec3(), new pc.Vec3(), new pc.Vec3(), new pc.Vec3()
    ];

    export function _getZFromAABBSimple(w2sc, aabbMin, aabbMax, lcamMinX, lcamMaxX, lcamMinY, lcamMaxY) {
    ...
    export function getShadowFormat(device, shadowType) {
    ...
    export function getShadowFiltering(device, shadowType) {
    ...
    export function createShadowMap(device, width, height, shadowType) {
    ...
    export function createShadowCubeMap(device, size) {
    ...
    export function gaussWeights(kernelSize) {
    ...
    export function createShadowCamera(device, shadowType, type) {
    ...
    export function getShadowMapFromCache(device, res, mode, layer) {
        if (!layer) layer = 0;
    ...
    export function createShadowBuffer(device, light) {
        var shadowBuffer;
    ...
    export function getDepthKey(meshInstance) {
    ...
    export function mat3FromMat4(m3, m4) {
        m3.data[0] = m4.data[0];
        m3.data[1] = m4.data[1];
        m3.data[2] = m4.data[2];
    ...
    export class ForwardRenderer {
        ...
        _isVisible(camera, meshInstance) {
        ...
        dispatchDirectLights(dirs, scene, mask) {
        ...
        dispatchPointLight(scene, scope, point, cnt) {
        ...
        clearView(camera, target, options) {
